...
/*nextRandomPos:
	; pseudo random number generator using Neuman (not working)
	lds POS_X, FRUITXADDR
	lds POS_Y, FRUITYADDR

	; random X position
	mul POS_X, POS_X ; result in R1:R0
	; shift right R0 4 times
	lsr R0
	lsr R0
	lsr R0
	lsr R0
	; shift left R1 4 times
	lsl R1
	lsl R1
	lsl R1
	lsl R1
	; we get the middle bits of the product
	add R1,R0
	mov RANDOM_POS, R1
	; apply modulo 80 for x pos
	rcall randomXpos
	mov POS_X, RANDOM_POS

	; random Y position
	mul POS_Y, POS_Y ; result in R1:R0
	; shift right R0 4 times
	lsr R0
	lsr R0
	lsr R0
	...
	add R1,R0
	mov RANDOM_POS, R1
	; apply modulo 7 for y pos
	rcall randomYpos
	mov POS_Y, RANDOM_POS
